...
---
##### Reduction Phase
You basically have to perform an XOR with $P(x)$.
...
### Division (Manual calculation)
If you have to calculate inverse Polynomial by yourself, you have to use the EEA algorithm:
$$
a = P(x)\\
b = A(x)\\
q = \text{Quotient } \lfloor\frac{a}{b}\rfloor\\
s = \text{starts with }1\\
t = \text{starts with }0,\; \text{The final result is }A^{-1}(x)\\
\text{remainder}=(s\cdot a) + (t\cdot b)
$$
